...
std::unique_ptr<std::vector<std::vector<uint8_t>>> getFrames(const std::function<std::complex<long double>(std::complex<long double>)>& f) {
    auto frames = std::make_unique<std::vector<std::vector<uint8_t>>>(num_frames, std::vector<uint8_t>(width*height));
    //for every pixel, ...
    for(int i=0; i<width; i++) {
        for(int j=0; j<height; j++) {
            auto value = f(std::complex<long double>(x_min + ((long double) i/width)*(x_max-x_min), y_max + ((long double) j/height)*(y_min-y_max)));
...
int main()
{
    ...
    auto frame_duration = std::chrono::duration_cast<std::chrono::microseconds>(after_frames - before_frames);
    auto fwrite_duration = std::chrono::duration_cast<std::chrono::microseconds>(after_fwrite - before_fwrite);
